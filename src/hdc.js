...
export function importHdc(xmlText) {
  ...
  return normalizeCharacter({
    ...
    profile: {
      ...
      campaignUse:
        info?.getElementsByTagName("CAMPAIGN_USE")[0]?.textContent || "",
      appearance:
...
function exportNativeHdc(character,templateHdc){
  ...
  const unsupported=[];for(const [section,entries] of Object.entries(character.sections||{}))for(const entry of entries){if(section==="powers"&&(entry.mechanics?.modifiers||[]).length){unsupported.push(`${entry.name}: Power modifiers need a verified Hero Designer prototype`);continue;}if(section==="disadvantages"&&(entry.mechanics?.key||entry.xmlId)!=="unluck"){unsupported.push(`${entry.name}: selected Disadvantage options cannot yet be represented losslessly in native HDC`);continue;}if(!nativePrototype(section,entry,prototypes))unsupported.push(`${entry.name||entry.alias}: no verified ${section} HDC prototype`);}

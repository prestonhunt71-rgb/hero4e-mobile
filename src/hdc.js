...
export function importHdc(xmlText) {
  ...
  const sections=Object.fromEntries(sectionNames.map((sectionName)=>{ const section=root.getElementsByTagName(sectionName)[0]; const entries=section?Array.from(section.childNodes).filter((node)=>node.nodeType===1).map((node)=>{const rawXml=new XMLSerializer().serializeToString(node),mechanics=sectionName==="POWERS"?analyzeHdcPower(rawXml):sectionName==="SKILLS"?analyzeHdcSkill(rawXml,characteristics):sectionName==="DISADVANTAGES"?analyzeHdcDisadvantage(rawXml):sectionName==="EQUIPMENT"?analyzeHdcEquipment(rawXml):null;return { id:attribute(node,"ID")||crypto.randomUUID(), tag:node.tagName, xmlId:attribute(node,"XMLID"), name:attribute(node,"NAME")||attribute(node,"INPUT")||attribute(node,"DISPLAY")||attribute(node,"ALIAS")||attribute(node,"XMLID")||node.tagName, alias:attribute(node,"ALIAS"), option:attribute(node,"OPTION_ALIAS")||attribute(node,"OPTION"), levels:Number(attribute(node,"LEVELS",0)), baseCost:Number(attribute(node,"BASECOST",0)), notes:node.getElementsByTagName("NOTES")[0]?.textContent?.trim()||"", rawXml, mechanics };}):[]; return [sectionName.toLowerCase(),entries]; }));

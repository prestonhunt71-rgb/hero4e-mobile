...
export function importHdc(xmlText) {
  ...
  const info = root.querySelector(":scope > CHARACTER_INFO");
  const block = root.querySelector(":scope > CHARACTERISTICS");
  if (!block) throw new Error("The HDC file has no CHARACTERISTICS block.");
  const characteristics = {};
  for (const key of primaryKeys) characteristics[key] = primaryDefinitions[key][0] + Number(attribute(block.querySelector(`:scope > ${key}`), "LEVELS", 0));
  const bases = figured(characteristics);
  for (const key of figuredKeys) characteristics[key] = bases[key] + Number(attribute(block.querySelector(`:scope > ${key}`), "LEVELS", 0));
